...
document.getElementById('btn-start').onclick = () => { 
    if (currentRoomCode) socket.emit('startGame', { code: currentRoomCode, categoria: 'lugares' }); 
};
...
function renderGame(data) {
    const customAInput = document.getElementById('custom-word-a');
    const customBInput = document.getElementById('custom-word-b');
    const savedCustomA = customAInput ? customAInput.value : '';
    const savedCustomB = customBInput ? customBInput.value : '';
    ...
    if (data.isHost) {
        // Render base del host (Código de sala y botón de cambio de modo)
        let dashboardHTML = `
            <div style="text-align: center; margin-bottom: 15px; display: flex; justify-content: center; align-items: center; gap: 10px;">
                <span class="room-code-badge">SALA: ${currentRoomCode}</span>
                <button class="btn-primary btn-sm" onclick="toggleHostMode()" style="background: #ff9800; padding: 6px 12px !important; margin: 0;">
                    MODO: ${data.hostIsPlaying ? 'JUGADOR' : 'MODERADOR'}
                </button>
            </div>
            <div class="admin-dashboard">
                <div class="admin-window">
                    ...
                    <div class="admin-scroll-list">
                        ${data.allPlayers.filter(p => p.username !== localStorage.getItem('player_name')).map(p => `
        ...
        if (!data.hostIsPlaying) {
            // Ventana de Info Secreta (Sólo si es Moderador)
            dashboardHTML += `
                <div class="admin-window">
            ...
            // Ventana de Rol Propio (Sólo si es Jugador)
            const impostorWordDisplay = (data.role === 'IMPOSTOR' && !data.wordBVisible) ? "???" : data.palabra.toUpperCase();
            dashboardHTML += `
                <div class="admin-window" style="justify-content: center;">
                    <h4 class="window-title">MI ROL SECRETO</h4>
                    <div id="role-box" class="blur-content" onclick="toggleVisibility('role-box')" style="min-height: 80px; margin-bottom: 15px;">
                        <p class="tap-hint">VER MI ROL</p>
                        <div class="real-content">
                            <h2 class="${data.role.toLowerCase()}" style="font-size:1.2rem; margin:0;">ERES ${data.role}</h2>
                        </div>
                    </div>
                    <div id="word-box" class="blur-content" onclick="toggleVisibility('word-box')" style="min-height: 80px;">
                        <p class="tap-hint">VER PALABRA SECRETA</p>
                        <div class="real-content">
                            <h1 id="display-word-text" class="word-highlight ${(!data.wordBVisible && data.role === 'IMPOSTOR') ? 'disabled-word' : ''}" style="font-size:1.4rem; margin:0;">${impostorWordDisplay}</h1>
                        </div>
                    </div>
                </div>
            `;
        ...
        // Ventana compartida de configuración de la dirección del juego
        dashboardHTML += `
                <div class="admin-window" style="grid-column: span 2; height: auto; gap: 10px;">
                    <h4 class="window-title">NUEVA RONDA / PALABRAS</h4>
                    
                    <select id="category-select" class="custom-select" onchange="checkCategoryPassword(this)">
                        <option value="lugares" ${data.currentCategory === 'lugares' ? 'selected' : ''}>Categoría: Lugares Comunes</option>
                        <option value="profesiones" ${data.currentCategory === 'profesiones' ? 'selected' : ''}>Categoría: Profesiones</option>
                        <option value="comida" ${data.currentCategory === 'comida' ? 'selected' : ''}>Categoría: Gastronomía</option>
                        <option value="deportes" ${data.currentCategory === 'deportes' ? 'selected' : ''}>Categoría: Deportes</option>
                        <option value="argentina" ${data.currentCategory === 'argentina' ? 'selected' : ''}>Categoría: Cultura Argentina</option>
                        <option value="futbol" ${data.currentCategory === 'futbol' ? 'selected' : ''}>Categoría: Fútbol</option>
                        <option value="cine" ${data.currentCategory === 'cine' ? 'selected' : ''}>Categoría: Películas y Series</option>
                        <option value="videojuegos" ${data.currentCategory === 'videojuegos' ? 'selected' : ''}>Categoría: Videojuegos</option>
                        <option value="cumple_jorge" ${data.currentCategory === 'cumple_jorge' ? 'selected' : ''}>Categoría: Cumpleaños de Jorge 🔒</option>
                    </select>
                    ...
                    <button class="btn-primary" onclick="sendCustomWords()" style="padding: 12px; font-size: 0.75rem; background: #3f51b5;">JUGAR PALABRAS PERSONALIZADAS</button>
                </div>
            </div>
        ...
        document.getElementById('custom-word-a').value = savedCustomA;
        document.getElementById('custom-word-b').value = savedCustomB;
...
window.checkCategoryPassword = (selectObj) => {
    if (selectObj.value === 'cumple_jorge') {
        const pass = prompt("Ingresá la contraseña para acceder a esta categoría:");
        if (pass !== '1805') {
            alert("CONTRASEÑA INCORRECTA");
            selectObj.value = 'lugares'; 
        }
    }
};
...
window.nextRound = () => { 
    const cat = document.getElementById('category-select') ? document.getElementById('category-select').value : 'lugares';
    socket.emit('nextRound', { code: currentRoomCode, categoria: cat }); 
};

window.leaveRoom = () => { 
    if (currentRoomCode) {
        socket.emit('playerLeft', currentRoomCode);
    }
    localStorage.clear(); 
    location.reload(); 
};
...
window.sendCustomWords = () => {
    ...
    socket.emit('startCustomRound', { code: currentRoomCode, palabraA, palabraB, categoria: cat });
};
...
window.changeImpostorsInGame = (delta) => { if (currentRoomCode) socket.emit('changeImpostorCount', { code: currentRoomCode, delta }); };

// NUEVO: Emitir el cambio de modo
window.toggleHostMode = () => { if (currentRoomCode) socket.emit('toggleHostMode', currentRoomCode); };
