...
function renderGame(data) {
    ...
    if (data.role === 'ADMIN') {
        content.innerHTML = `
            ...
            <button class="btn-primary" onclick="reassignRoles()" style="margin-top:10px; background: #3f51b5; box-shadow: 0 10px 20px rgba(63, 81, 181, 0.3);">REASIGNAR ROLES</button>
...
window.sendCustomWords = () => {
    ...
    if (!palabraA || !palabraB) return alert("POR FAVOR, ESCRIBÍ AMBAS PALABRAS");
    
    socket.emit('startCustomRound', {
...
window.reassignRoles = () => {
    if (currentRoomCode) socket.emit('reassignRoles', currentRoomCode);
};
