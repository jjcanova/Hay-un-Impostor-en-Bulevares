...
function renderGame(data) {
    ...
    if (data.role === 'ADMIN') {
        content.innerHTML = `
            <div class="setup-box">
                <h3 class="panel-title" style="color:#ffca28">MODERADOR</h3>
                <div class="admin-info">
                    <p><strong>REVELACIÓN:</strong><br>${data.palabra}</p>
                </div>
                <div class="admin-grid">
                    ${data.allPlayers.filter(p => p.role !== 'ADMIN').map(p => `
                        <div class="player-card admin-view" id="card-${p.id}" onclick="toggleEliminar('${p.id}')">
                            ...
                            <div class="${p.role.toLowerCase()}" style="font-size:0.6rem">${p.role}</div>
                        </div>
                    `).join('')}
                </div>
                <button class="btn-primary" onclick="nextRound()" style="margin-top:15px">NUEVA RONDA</button>
        ...
        content.innerHTML = `
            ...
            <div id="game-phase-debate" class="hidden">
                <div class="setup-box" style="text-align:center">
                    ...
                    <p style="font-size:0.7rem; color:#8e90a5">El Moderador está supervisando la partida.</p>
...
window.toggleEliminar = (id) => document.getElementById(`card-${id}`).classList.toggle('eliminado');
